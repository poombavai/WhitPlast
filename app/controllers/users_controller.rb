...
#skip_before_action :check_session, :only=>[:login,:validate_login]
  skip_before_action :check_session, :current_user, :only=>[:login,:validate_login,:create]
  $pend_order = []
  ...
  def new
    ...
    @users = User.all#.where(branch_id: current_user.branch_id)
#    @branch = User.branch_ids
#    @role = User.roles
  end
...
#       redirect_to :action => "new"
  ...
  def index_orderSummary
    ...
    if Date.today.month>=4
      @order_summary = OrderSummary.where(:created_at=>("#{Time.now.year-1}-04-01")..("#{Time.now.year}-03-31")).order('order_no DESC')
    else
      @order_summary = OrderSummary.where(:created_at=>("#{Time.now.year-1}-04-01")..("#{Time.now.year}-03-31")).order('order_no DESC')
    end
    $pend_order = @order_summary
    if params[:search] && Date.today.month >= 4
      @order_summary = OrderSummary.where(:created_at=>("#{Time.now.year-1}-04-01")..("#{Time.now.year}-03-31")).order('order_no DESC')
      @order_summary =  @order_summary.search(params[:search]).paginate(page: params[:page]).per_page(10)
      render :action => :index_orderSummary
    elsif params[:search] && Date.today.month < 4
      @order_summary = OrderSummary.where(:created_at=>("#{Time.now.year-1}-04-01")..("#{Time.now.year}-03-31")).order('order_no DESC')
      @order_summary =  @order_summary.search(params[:search]).paginate(page: params[:page]).per_page(10)
      render :action => :index_orderSummary
    end
  ...
  def order_summary_report
    ...
    @order_summary=OrderSummary.where(:order_date=>$oss..$ose).order('order_no DESC')#.select(@fl.join(","))
  ...
  def order_sum_report
    @order_summary=OrderSummary.where(:order_date => $oss..$ose).order('order_no DESC')
    $ohd,$ofd=$oshd,$osfd
  end
  ...
  def index_productionReport
    ...
    if params[:search] && Date.today.month >= 4 
      @production_report = ProductionReport.where(:created_at=>("#{Time.now.year-1}-04-01")..("#{Time.now.year}-03-31")).order('issue_slip_no DESC')
      @production_report =  @production_report.search(params[:search]).paginate(page: params[:page]).per_page(10).order('issue_slip_no DESC')
...
#   @production_report.destroy
  ...
  def productn_report
    ...
    @labour=Labour.where(:date=>$prs..$pre)
  ...
  def product_report
    @production_report=ProductionReport.where(:date => $prs..$pre).order('issue_slip_no DESC')
    $prod1=$prod
  end
  ...
  def index_purchase
    ...
    @user = User.find(session[:user_id]).name
    if Date.today.month>=4
      @purchase = Purchase.where(:created_at=>("#{Time.now.year-1}-04-01")..("#{Time.now.year}-03-31")).order("grn_no ASC")
    else
      @purchase = Purchase.where(:created_at=>("#{Time.now.year-1}-04-01")..("#{Time.now.year}-03-31")).order("grn_no ASC")
    end
    if params[:search] && Date.today.month >= 4
  ...
  def purchase_report
    ...
    @purchase=Purchase.where(:date=>$c..$d).order("grn_no ASC")#.select(@fl.join(","))
  ...
  def get_issues
    ...
    @issue.order_no = params[:issue][:order_no].split("/").last.strip
    if @issue.save
  ...
  def update_issues
    ...
    @user = User.find(session[:user_id]).name
    @issue = Issue.find params[:id]
    issues_params[:order_no] = issues_params[:order_no].split('-').last
    issues_params[:issue_slip_no] = issues_params[:issue_slip_no].split('-').last
...
#            @order_summary = OrderSummary.find_by("id LIKE '%#{@o_id}%'")
          ...
          def update_labour
           ...
           # @labour = Labour.find_by ("production_report_id LIKE '%#{params[:id]}%'")
            @labour = Labour.find(labour_params[:id])
...
def report_oee
  ...
  def qty_after_rg
    ...
    def pending_order
      ...
      def report_color_agent_issue
       @user=User.new
       @user = User.find(session[:user_id]).name
      end

      def color_agent_issue
        @user = User.find(session[:user_id]).name
        @bb = params[:issue][:intial_date]
        @aa = params[:issue][:final_date]
        @issue =  Issue.where(:issue_date => @bb..@aa).select("issue_date","chemicals","shift","issue_slip_no","chemicals_type","che_qty")
        @value = Issue.where(:issue_date => @bb..@aa).group(:chemicals).sum(:che_qty)
      end

      def report_reground_usage
       @user=User.new
       @user = User.find(session[:user_id]).name
      end
      ...
      def reground_usage
        ...
        @bb = params[:issue][:intial_date]
        @aa = params[:issue][:final_date]
        @issue = Issue.where(:issue_date => @bb..@aa).select("issue_date","rg_material_issues","rg_qty","shift","issue_slip_no","chemicals")
        @value = Issue.where(:issue_date => @bb..@aa).group(:rg_material_issues).sum(:rg_qty)

      end

      def report_finished_goods
       @user=User.new
       @user = User.find(session[:user_id]).name
      end
      ...
      def finished_goods
        ...
        @bb = params[:issue][:intial_date]
        @aa = params[:issue][:final_date]
        @issue = ProductionReport.where(:issue_date => @bb..@aa).joins(:issue).select("issue_date","total_weight_consumed","shift","finished_goods_name","party","total_no_of_items_produced","issue_slip_no","chemicals","rm_issues")
        @value = ProductionReport.where(:issue_date => @bb..@aa).group(:finished_goods_name).sum(:total_weight_consumed)
        @value1= ProductionReport.where(:issue_date => @bb..@aa).group(:finished_goods_name).sum(:total_no_of_items_produced)
      end

      def report_raw_material_usage
       @user=User.new
       @user = User.find(session[:user_id]).name
      end
      ...
      def raw_material_usage
        ...
        @bb = params[:issue][:intial_date]
        @aa = params[:issue][:final_date]
        @issue = Issue.where(:issue_date => @bb..@aa).select("rm_issues","rm_qty","issue_slip_no","issue_date","shift")
        @value = Issue.where(:issue_date => @bb..@aa).group(:rm_issues).sum(:rm_qty)
